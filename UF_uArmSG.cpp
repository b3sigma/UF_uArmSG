...
void UF_uArm::setPosition(double _stretch, double _height, int _armRot,
    int _handRot) {
  _armRot = -_armRot;
  ...
  if (angleL < (10 + offsetL)) {
    angleR = angleR < (80 + offsetR) ? (80 + offsetR) : angleR;
  }
  angleR =
      angleR < (angleL - 95 + 30 + offsetR) ?
          (angleL - 95 + 30 + offsetR) : angleR;
...
void UF_uArm::sendData(byte _dataAdd, int _dataIn) {
  Serial.write(0xFF);
  Serial.write(0xAA); // send data head
  Serial.write(_dataAdd);
  Serial.write(*((char *) (&_dataIn) + 1));
  Serial.write(*((char *) (&_dataIn)));
}

void UF_uArm::notify(unsigned long _durationMs) {
#ifdef BUZZ_NOTIFICATION
